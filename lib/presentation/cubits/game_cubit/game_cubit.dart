...
class GameCubit extends Cubit<GameState> {
  ...
  final GetBestResultsForAlien getBestResultsForAlien = sl<GetBestResultsForAlien>();
  final SetBestResultsForAlien setBestResultsForAlien = sl<SetBestResultsForAlien>();
    ...
    if (key == PhysicalKeyboardKey.arrowUp) {
      if (emptyPosition.y + 1 <= crossAxisCount - 1) {
        index = tiles.indexWhere(
          (tile) {
            return tile.currentPosition.x == emptyPosition.x && tile.currentPosition.y == emptyPosition.y + 1;
          },
        );
      }
      ...
      if (emptyPosition.x <= crossAxisCount) {
        index = tiles.indexWhere(
          (tile) {
            return tile.currentPosition.x + 1 == emptyPosition.x && tile.currentPosition.y == emptyPosition.y;
          },
        );
      }
  ...
  Future<void> addAlienToAlbum(int time) async {
    ...
    final bestMoves = getBestResultsForAlien(alienName: '${state.alienName}_results').first;
    if (bestMoves == 0) {
      await setBestResultsForAlien(alienName: '${state.alienName}_results', values: [state.moves, time]);
    } else if (bestMoves > state.moves) {
      await setBestResultsForAlien(alienName: '${state.alienName}_results', values: [state.moves, time]);
    }
  }

  void pause() => emit(state.copyWith(status: state.status == GameStatus.paused ? GameStatus.playing : GameStatus.paused));
