...
class DifficultyView extends StatelessWidget {
                ...
                if (state is DifficultLoading || state is DifficultLoaded)
                  Center(
                    child: Container(
                        ...
                        decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), border: Border.all(width: 3, color: border), color: Colors.black.withOpacity(0.3)),
  ...
  Widget _buildBody(Size size, BuildContext context) {
    const duration = 800;
    final size = MediaQuery.of(context).size;
    final constraints = BoxConstraints(maxWidth: (size.width / 2).clamp(200, 300), minHeight: 60);
    return SafeArea(
      child: Column(
        children: [
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      SpaceButton(
                        onPressed: () => _loadAlienAssets(context, GameDifficult.easy),
                        title: AppLocalizations.of(context).difficultEasy,
                        constraints: constraints,
                      ),
                      const SizedBox(height: 25),
                      SpaceButton(
                        onPressed: () => _loadAlienAssets(context, GameDifficult.medimum),
                      ...
                      const SizedBox(height: 25),
                      SpaceButton(
                        onPressed: () => _loadAlienAssets(context, GameDifficult.hard),
                      ...
                      const SizedBox(height: 25),
                      SpaceButton(
                        onPressed: () => _loadAlienAssets(context, GameDifficult.extremeLevel),
                        title: AppLocalizations.of(context).difficultExtreme,
  ...
  void _loadAlienAssets(
    ...
    Timer(const Duration(milliseconds: 50), () => context.read<DifficultCubit>().loadAssets(gameDifficult));
  }

  Future<void> _pushGameView(BuildContext context, GameParams gameParams) async {
    ...
    if (context.mounted) context.read<DifficultCubit>().initialState();
  }
}
